...
fn tokenizer(source_code: &str) -> Vec<Token> {
    let re = Regex::new(
        r#"var\s|if\s|else\s|while\s|==|!=|!|=|<|>|\+|-|\*|\{|\}|[|]|\(|\)|\strue\s|\sfalse\s|,|\d+.?\d*|"(\w+\s*)*"|\n|\w+"#,
    )
    .unwrap();
    re.captures_iter(source_code).inspect(|c|println!("{:?}",c)).map(|cap|cap.get(0).unwrap().into())
        .map(|cap : &str| match cap.trim() {
